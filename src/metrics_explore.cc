...
#include "ncode_web/src/mongoose.h"
#undef LOG
...
#include "ncode_web/src/grapher.h"
...
static void DumpData(
  ...
  for (const auto& ids_and_data : data) {
    ...
    std::function<std::string(const std::pair<uint64_t, double>&)> f = [](
      ...
      return StrCat(timestamp_and_value.first, " ", timestamp_and_value.second);
    };

    std::string to_write = StrCat(ids.first, " ", ids.second, " ");
    Join(values.begin(), values.end(), " ", f, &to_write);
    out << to_write << "\n";
  }
...
class ServerState {
 public:
  ServerState(const std::string& file_or_dir) {
    bool is_dir;
    CHECK(File::FileOrDirectory(file_or_dir, &is_dir));

    std::vector<std::string> files;
    if (is_dir) {
      files = Glob(StrCat(file_or_dir, "/*"));
    } else {
      files.emplace_back(file_or_dir);
    }

    for (const std::string& file : files) {
      uint64_t file_size_mb = File::FileSizeOrDie(file) / 1000 / 1000;
...
static std::string ExtractURLVar(const std::string& id,
                                 const mg_request_info& request_info) {
  const char* qs = request_info.query_string;
  CHECK(qs != nullptr);

  char var[1024];
  int var_len = mg_get_var(qs, strlen(qs == NULL ? "" : qs), id.c_str(), var,
                           sizeof(var));
  CHECK(var_len != -2) << "Variable too large";
  if (var_len == -1) {
    return "";
  }
...
static std::string ExtractPostDataVar(const std::string& id,
                                      mg_connection* connection) {
  const struct mg_request_info& ri = connection->request_info;

  char var[1024];
  int post_data_len = mg_read(connection, var, sizeof(var));
}

// Extracts a get/post variable from the http message or returns "".
static std::string ExtractVariable(const std::string& id,
  ...
  res =
      mg_get_var(request_info->query_string, strlen(request_info->query_string),
                 id.c_str(), var, sizeof(var));
  if (res == -2) {
    LOG(ERROR) << "Buffer too small (GET) for " << id;
  }

  if (res == 0 || res == -1 || res == -2) {
    res = mg_get_var(request_info->conn_data, strlen(request_info->conn_data),
                     id.c_str(), var, sizeof(var));
    if (res == -2) {
...
static grapher::PlotParameters2D Extract2DPlotParameters(http_message* hm) {
  grapher::PlotParameters2D parameters;
...
static grapher::PlotParameters1D Extract1DPlotParameters(http_message* hm) {
  grapher::PlotParameters1D parameters;
...
static bool ExtractBoolOrFalse(const std::string& value) {
  bool return_value;
  if (!safe_strtob(value, &return_value)) {
    return false;
  }
  return return_value;
}

static void Handler(mg_connection* connection) {
  const mg_request_info* request_info = mg_get_request_info(connection);

  ServerState* server_state =
      reinterpret_cast<ServerState*>(request_info->conn_data);
  ...
  if (mg_vcmp(&hm->uri, "/index") == 0) {
    ...
    std::vector<std::string> ids = ManifestToHtml(file.manifest, i, page.get());
    if (ids.size() == 1) {
...
static std::set<uint32_t> ExtractManifestIds(http_message* hm,
  ...
  std::vector<std::string> pieces =
      Split(ids_string.substr(1, ids_string.size() - 2), ",");
  std::set<uint32_t> manifest_ids;
  uint32_t value;
  for (const std::string& piece : pieces) {
    if (!safe_strtou32(piece, &value)) {
